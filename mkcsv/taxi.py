...
def int_to_datetime(i):
    year = int(i/10000000000)
    month = int(i/100000000)-(year*100)
    day = int(i/1000000) - (int(i/100000000)*100)
    hour = int(i/10000) - (int(i/1000000)*100)
    minute = int(i/100) - (int(i/10000)*100)
    second = i - (int(i/100)*100)

    return datetime.datetime(year,month,day,hour,minute,second)

def foot_of_perpendicular(line1_x,line1_y,line2_x,line2_y,point_x,point_y):
    x = line2_x - line1_x
    y = line2_y - line1_y
    p_x = point_x - line1_x
    p_y = point_y - line1_y
    if x == 0 :
        return(line1_x,point_y)
    elif y == 0 :
        return(point_x,line1_y)
    else :
        return(line1_x+(p_y+x/y*p_x)/(y/x+x/y),line1_y+(p_y*y/x+p_x)/(y/x+x/y))

def gps_to_cell(origin,gps):
    origin = (int(origin[0]),int(origin[1]))
    gps = (int(gps[0]),int(gps[1]))
    dx = gps_to_d(origin[0],origin[1],gps[0],origin[1])
    dy = gps_to_d(origin[0],origin[1],origin[0],gps[1])
    nx = int(dx/100)
    ny = int(dy/100)
    pmx = 1
    pmy = 1
...
def exp_kml_poly(lon,lat,d,file_name='defult',poly_name='defult',poly_script='empty'):
    print("output4/{}.kml_poly".format(file_name))
    fp = open("output4/{}.kml".format(file_name),"a")
    fp.write('''    <Placemark>
      <name>{}</name>
      <description>{}</description>
